...
### Core
Shared vocabulary and abstract ports: prayer/display/theme value objects (frozen) incl. marker vocabulary (`MarkerName`, `MarkerKind` + `marker_kind`), `PrayerRepo`, `SettingsRepo`, `DisplayRepo`, `UserRepo`, `JAKIMClient`, `CalcEngine`, `EventBus`, `MediaStore`, `Clock` ports, exception hierarchy (`MuhideenError`, `ContractError`, `ScheduleError`, `SyncError`). No framework, no SQLite, no HTTP.

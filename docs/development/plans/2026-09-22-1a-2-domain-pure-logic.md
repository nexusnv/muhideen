...
- [ ] Extend `resolve_next_event`: Syuruq slot gets `ADHAN` overlay for `settings.adhan_duration_s` then falls through to `NORMAL` (skip countdown/dim branches); Friday detection via `now.date().weekday() == 4` relabels the Dhuhr slot to `JUMUAH` and selects `settings.dim_minutes_jumuah` for its dim length; schedule clock times anchor on `now.date()` so a last-known day from another date (Task 2 allows any date) renders as a template instead of emitting a past `adhan_at`; post-Isha-dim `now` resolves to next-day Fajr per the synthesis rule above. Verify: same command → PASS; `uv run pytest -m unit -q` → PASS.

**As-built (oracle round):** Friday detection and midnight anchor read `now.date()`, not `today.date()` as first drafted — required so the offline last-known path never returns `adhan_at` in the past. All Task 5 vectors pin `today.date == now.date()`, so asserted outcomes are unchanged.
...
- [ ] Add failing-then-passing determinism test `test_same_inputs_identical_output`: fixed `now`, fixed `PrayerDay` snapshot, fixed `Settings`, fixed rules dict → call `resolve_next_event` twice and assert full `NextEvent` equality (all seven fields); second case replays the `api/fixtures/next-event.json:1` vector (`2025-10-20T12:20:00+08:00` → `IQAMAH_COUNTDOWN` dhuhr; corrected from `11:45` in the oracle round, see checklist item 2) twice and asserts identical output. File-local `FakeClock` only. Run: `uv run pytest tests/unit/test_domain_determinism.py -v` → Expected: FAIL before `__init__` exports exist (import path), then PASS after.
...
2. **Contract diff:** `shape: none`, one example-value correction. `api/fixtures/next-event.json` + `docs/api-contract.md` example `now` corrected `11:45` → `12:20`: the old vector claimed `IQAMAH_COUNTDOWN` before `adhan_at`, unreproducible under PRD §8 (oracle round). No key/shape change; no CHANGELOG file exists yet — 1A-3 owns parity and the changelog convention.
